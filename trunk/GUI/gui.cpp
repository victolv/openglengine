...
bool gui::newElem(basicElem * newElement)
{
	...
	if(autoSize)
	{
		scale(0,newElement->posY + newElement->sizeX + 5);
		if(newElement->sizeX > sizeX)
		sizeX = newElement->sizeX + 40;
	}

	if(newElement->getX() == 0 && newElement->getY() == 0)
	{
	/*betekend dat er geen coordinaten zijn opgegeven, 
	maar die kunne wij ook wel genereren :) */
		int tempY = 0;
		if(elems.empty())
		{
			newElement->move(0,posY + 5);
		}
		for(list<basicElem*>::iterator current = elems.begin();current != elems.end();current++)
		{

			while(!((*current)->getX() != newElement->getX() &&
				(*current)->getY() + (*current)->sizeY + 5 < newElement->getY() &&
				(*current)->getY() < newElement->getY() - 5))
			{
				newElement->move(0,1);
			tempY = newElement->posY + newElement->sizeY;
			}

		}
		
		if(tempY > posY + sizeY - 5)
			return false;


	}
	newElement->move(posX + 10, 5);
	sizeY = newElement->posY - posY + 40;
